...
<form action="{{route('updateemployer')}}" method="post" class="row g-3" style="display:flex;flex-direction:column">
    <input type="hidden" name="id" value="{{$data->id}}">
    @csrf
    <div class="col-md-4">
        <label for="fullnameInput" class="form-label">@lang('211')</label>
        <input disabled="true" value="{{$data->username}}"  required="true" type="text" class="form-control" id="fullnameInput" placeholder="Enter your name">
    </div>
    <div class="col-md-4">
    ...
    <div class="col-md-4">
        ...
        <select name="role" required="true" id="inputState" class="form-select">
            @foreach($role as $item)
                            <option value="{{$item->id}}" @if($item->id == $data->role_group) selected @endif >{{$item->name}}</option>
    ...
    <div class="col-4">
        <div class="text-end">
            <button type="submit" class="btn btn-primary">@lang("126")</button>
        </div>
    </div>
</form>
                    </div>
